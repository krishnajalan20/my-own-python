...
def broad_2(msg, id):
    ...
    text = f"*✔️ Broadcast Sended To All Users\n\n✅ Success : {success} *"
...
def with_2(id, amo):
    ...
    if float(amo) < m_with:
        ...
        bot.send_message(id, f"*⚠️ 𝙼𝚒𝚗𝚒𝚖𝚞𝚖 𝚆𝚒𝚝𝚑𝚍𝚛𝚊𝚠 𝙸𝚜 {m_with} {curr}*", parse_mode="Markdown")

        return

    if float(amo) > bal:

        bot.send_message(id, "*🚫 𝙴𝚗𝚝𝚎𝚛𝚎𝚍 𝙰𝚖𝚘𝚞𝚗𝚝 𝙸𝚜 𝙶𝚛𝚎𝚊𝚝𝚎𝚛 𝚃𝚑𝚊𝚗 𝚈𝚘𝚞𝚛 𝙱𝚊𝚕𝚊𝚗𝚌𝚎*", parse_mode="Markdown")
...
def with_1(message):
    ...
    try:
        ...
        if float(amo) < m_with:
            ...
            bot.send_message(id, f"*⚠️ 𝙼𝚒𝚗𝚒𝚖𝚞𝚖 𝚆𝚒𝚝𝚑𝚍𝚛𝚊𝚠𝚕 𝙸𝚂 {m_with} {curr}*", parse_mode="Markdown")
        ...
        if float(amo) > bal:
            ...
            bot.send_message(id, "*⛔ Entere𝚍 Amount Is Greater Than Your Balance*", parse_mode="Markdown")
...
def menu(id):
    ...
    keyboard.row('💵 𝙱𝙰𝙻𝙰𝙽𝙲𝙴','💎 𝙱𝙾𝙽𝚄𝚂')

    keyboard.row('🛡 𝙸𝙽𝚅𝙸𝚃𝙴','🔐 𝚆𝙰𝙻𝙻𝙴𝚃')

    keyboard.row('🏧 𝙿𝙰𝚈𝙾𝚄𝚃', '📈 𝚂𝚃𝙰𝚃𝙸𝚂𝚃𝙸𝙲𝚂')

    bot.send_message(id, "*🏘️ 𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 𝙼𝙰𝙸𝙽 𝙼𝙴𝙽𝚄*", parse_mode="Markdown", reply_markup=keyboard)
...
def callbck_query(call):
    ...
    if call.data == 'Per_r':
        ...
        bot.answer_callback_query(callback_query_id=call.id, text="✅ 𝙿𝚁𝙾𝙲𝙴𝚂𝚂𝙸𝙽𝙶 𝚈𝙾𝚄𝚁 𝚆𝙸𝚃𝙷𝙳𝚁𝙰𝚆𝙻 𝚁𝙴𝚀𝚄𝙴𝚂𝚃",
...
def send_start(user):
    ...
    msg_start = "*💡You Have To Join Our All Channel\n\n"
...
def send_text(message):
    ...
    if verify == "Not":
        ...
        if date_user == date:
            ...
            text = '*⛔️ 𝙱𝙾𝙽𝚄𝚂 𝚁𝙴𝙲𝙸𝙴𝚅𝙴𝙳 𝙰𝙻𝚁𝙴𝙰𝙳𝚈 - 𝙲𝙷𝙴𝙲𝙺 𝙰𝙵𝚃𝙴𝚁 24 𝙷𝙾𝚄𝚁𝚂*'

        else:

            text = f'*🤑 𝙲𝙾𝙽𝙶𝚁𝙰𝚃𝚂 , 𝚈𝙾𝚄 𝙶𝙾𝚃 {bonus} {curr}\n\n🔎 𝙲𝙷𝙴𝙲𝙺 𝙱𝙰𝙲𝙺 𝙰𝙵𝚃𝙴𝚁 24 𝙷𝙾𝚄𝚁𝚂*'
        ...
        if bal < m_with:
            ...
            bot.send_message(id, f"*⚠️ 𝙼𝚄𝚂𝚃 𝙾𝚆𝙽 𝙼𝙸𝙽. {m_with} {curr} 𝚃𝙾 𝙼𝙰𝙺𝙴 𝚆𝙸𝚃𝙷𝙳𝚁𝙰𝚆𝙻*", parse_mode="Markdown")

        elif wallet == "None":

            bot.send_message(id, "*⚠️ 𝚂𝙴𝚃𝚄𝙿 𝚈𝙾𝚄𝚁 𝚆𝙰𝙻𝙻𝙴𝚃 𝙱𝚈 : *`🔐 𝚆𝙰𝙻𝙻𝙴𝚃`", parse_mode="Markdown")

        else:

            msg = bot.send_message(user, "*📤 𝙴𝙽𝚃𝙴𝚁 𝙰𝙼𝙾𝚄𝙽𝚃 (𝙽𝙾𝚃 𝙸𝙽 𝙳𝙴𝙲𝙸𝙼𝙰𝙻 𝙾𝚁𝙳𝙴𝚁)*", parse_mode="Markdown")
